...
glm::vec2 MousePicker::getNormalizedDeviceCoords(float mouseX, float mouseY) {
    float x = (2.0f * mouseX) / (float)DisplayManager::Width() - 1;
    float y = (2.0f * mouseY) / (float)DisplayManager::Height() - 1;
    return glm::vec2(x, -y);
}

glm::vec3 MousePicker::calculateMouseRay() {
    glm::vec2 normalizedCoords = getNormalizedDeviceCoords((float)CameraInput::MouseX, (float)CameraInput::MouseY);
