...
SkyboxRenderer::SkyboxRenderer(Loader *loader, glm::mat4 projectionMatrix, Color *skyColor)  : sBoxVertices(initVertices(sBoxSize)), sTextureFiles(
...
void SkyboxRenderer::render(Camera *camera) {
    OpenGLUtils::enableDepthWriting(false);
    shader->start();
    ...
    glBindVertexArray(0);
    shader->stop();
    OpenGLUtils::enableDepthWriting(true);
}
...
void SkyboxRenderer::bindTextures() {
    ...
    if(time >= 0 && time < 5000){
        ...
        *skyColor = ColorName::Skyblue;
        blendFactor = (time - 0)/(5000 - 0);
    }else if(time >= 5000 && time < 8000){
        texture1 = nightTextureId;
        texture2 = dayTextureId;
        blendFactor = (time - 5000)/(8000 - 5000);
        *skyColor = ColorName::Lightgrey;
    }else if(time >= 8000 && time < 21000){
        texture1 = dayTextureId;
        texture2 = dayTextureId;
        *skyColor = ColorName::Whitesmoke;
        blendFactor = (time - 8000)/(21000 - 8000);
    }else{
        texture1 = dayTextureId;
        texture2 = nightTextureId;
        *skyColor = ColorName::Black;
